              ...
              <h3>{{ $gimme->section->name }}</h3>
                <ul>
                    <li>
                        ...
                        <p class="article-info"><em>{{ 'publishedOn'|translate }} {{ $gimme->article->publish_date|camp_date_format:"%M %e, %Y" }} {{ 'in'|translate }} <a href="{{ uri options="section" }}">{{ $gimme->section->name }}</a></em><span class="right">{{ include file="_tpl/article-icons.tpl" }}</span></p>
