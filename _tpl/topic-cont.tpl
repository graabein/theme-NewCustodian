<h1>{{ 'Topic:'|translate }} {{ $gimme->topic->name }}</h1>
...
<ul class="pagination">
    ...
    <li class="selected"><a href="{{ uripath options="section" }}?{{ urlparameters }}">{{ $i+1 }}</a></li>
        ...
        {{ /if }}
    {{ /for }}
    {{ if $gimme->current_list->has_next_elements }}<li class="next"><a href="{{ uripath options="template topics.tpl" }}?{{ urlparameters options="next_items" }}">{{ 'next'|translate }}</a></li>{{ /if }}
</ul>
