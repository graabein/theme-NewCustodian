...
{{ /if }}
        ...
        <li class="registered">{{ include file="_tpl/relative_date.tpl" date=$created }} <a{{ if $user->is_active }} href="{{ $view->url(['username' => $user->uname], 'user') }}"{{ /if }}>{{ $user->first_name }} {{ $user->last_name }}</a> {{ 'registered'|translate }}</li>
        {{ elseif $gimme->community_feed->type == 'comment-recommended' && $gimme->community_feed->comment->article }}
        <li class="commented">{{ include file="_tpl/relative_date.tpl" date=$created }} {{ 'newCommentOn'|translate }} <a href="{{ $gimme->community_feed->comment->article->url }}#comments">{{ $gimme->community_feed->comment->article->title }}</a></li>
        {{ /if }}
