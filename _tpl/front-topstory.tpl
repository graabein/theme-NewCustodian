          ...
          {{ include file="_tpl/img/img_500x333.tpl" }}
              ...
              <header>
                  <hgroup>
                        ...
                        <p class="article-info"><em>{{ include file="_tpl/article-icons.tpl" }} {{ 'publishedOn'|translate }} {{ $gimme->article->publish_date|camp_date_format:"%M %e, %Y" }} {{ 'in'|translate }} <a href="{{ uri options="section" }}">{{ $gimme->section->name }}</a></em></p>
                    </hgroup>
                </header>
                <div id="topstory-intro">
                  <p>{{ include file="_tpl/_edit-article.tpl" }}{{ $gimme->article->deck }}</p>
                    <p><em>{{ list_article_authors }}{{ if $gimme->current_list->at_beginning }}{{ 'by'|translate }} {{ /if }}{{ if $gimme->current_list->at_end }}{{ if $gimme->current_list->index > 1 }} {{ 'and'|translate }} {{ /if }}{{ else }}{{ if $gimme->current_list->index > 1 }}, {{ /if }}{{ /if }}{{ if $gimme->author->user->defined }}<a href="{{ $view->url(['username' => $gimme->author->user->uname], 'user') }}">{{ /if }}{{ $gimme->author->name }}{{ if $gimme->author->user->defined }}</a>{{ /if }}{{ if $gimme->current_list->at_end }}. {{ /if }}{{ /list_article_authors }}</em></p>
                </div>
                </div>
            </section><!-- /#topstory -->
