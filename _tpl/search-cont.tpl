...
{{ if $pages gt 1 }}
<ul class="pagination">
    {{ if $gimme->current_list->has_previous_elements }}<li class="prev"><a href="{{ url options="template search.tpl previous_items" }}">{{ 'previous'|translate }}</a></li>{{ /if }}
    {{ for $i=0 to $pages - 1 }}
        {{ $curlistid=$i*5 }}
    ...
    <li class="selected"><a href="{{ url options="template search.tpl" }}">{{ $i+1 }}</a></li>
        ...
        {{ /if }}
    {{ /for }}
    {{ if $gimme->current_list->has_next_elements }}<li class="next"><a href="{{ url options="template search.tpl next_items" }}">{{ 'next'|translate }}</a></li>{{ /if }}
</ul>
