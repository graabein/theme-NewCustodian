...
{{ if $deadline->getTimestamp() > time() }}
    <p>{{ $diff->days }} {{ 'days'|translate }}, {{ $diff->h }} {{ 'hours'|translate }}, {{ $diff->i }} {{ 'minutes'|translate }} more {{ if $gimme->article->comment_count }}<span class="comm">{{ $gimme->article->comment_count }}</span>{{ /if }}</p>
{{ else }}
